...
// GstCoin with Governance.
contract GstCoin is BEP20('Gst Coin', 'GST') {
...
// MasterChef is the master of Gst. He can make Gst and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GST is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract GstMasterChefL3 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accGstPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGstPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accGstPerShare;  // Accumulated KRs per share, times 1e18. See below.
        uint16 depositFeeBP;        // Deposit fee in basis points
        uint256 harvestInterval;    // Harvest interval in seconds
    }

    // The GST TOKEN!
    GstCoin public immutable gst;
    // Dev address.
    address public devAddress;
    // GST tokens created per second.
    uint256 public gstPerSecond;
    // Deposit Fee address
    ...
    // The timestamp when GST mining starts.
    uint256 public startTime;
    
    // Maximum gstPerSecond
    uint256 public constant MAX_EMISSION_RATE = 0.1 ether;
    // Initial gstPerSecond
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed gstPerSecond);
    ...
    constructor(
        ...
        gstPerSecond = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's GST allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingGst(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accGstPerShare = pool.accGstPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 gstReward = multiplier.mul(gstPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accGstPerShare = accGstPerShare.add(gstReward.mul(1e18).div(lpSupply));
        }

        uint256 pending = user.amount.mul(accGstPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }


    // View function to see if user can harvest Gsts's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (gst.totalSupply() >= MAX_SUPPLY) {
            gstReward = 0;
        } else if (gst.totalSupply().add(gstReward.mul(11).div(10)) >= MAX_SUPPLY) {
            gstReward = (MAX_SUPPLY.sub(gst.totalSupply()).mul(10).div(11));
        }

        if (gstReward > 0) {
            gst.mint(devAddress, gstReward.div(10));
            gst.mint(address(this), gstReward);
            pool.accGstPerShare = pool.accGstPerShare.add(gstReward.mul(1e18).div(lpSupply)); 
        }

        pool.lastRewardTime = block.timestamp;
    }

    // Deposit LP tokens to MasterChef for GST allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        updatePool(_pid);
        payOrLockupPendingGst(_pid);
        ...
        user.rewardDebt = user.amount.mul(pool.accGstPerShare).div(1e18);
    ...
    // Pay or lockup pending gst.
    function payOrLockupPendingGst(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accGstPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = getPoolHarvestInterval(_pid);
                // send rewards
                safeGstTransfer(msg.sender, rewardsToDistribute);
            }
        } else if (pending > 0) {
    ...
    // Safe gst transfer function, just in case if rounding error causes pool to not have enough KRs.
    function safeGstTransfer(address _to, uint256 _amount) internal {
        uint256 gstBal = gst.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > gstBal) {
            transferSuccess = gst.transfer(_to, gstBal);
        } else {
            transferSuccess = gst.transfer(_to, _amount);
        }
        require(transferSuccess, "safeGstTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _gstPerSecond) external onlyOwner {
        require (_gstPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: value higher than maximum");
        massUpdatePools();
        gstPerSecond = _gstPerSecond;
        emit UpdateEmissionRate(msg.sender, _gstPerSecond);
    }
